...
class UserPersistFileDao {
     ...
     * @param $objList save all users
     * @return $arrayUsers array of users found
     */
    public function searchUsersByUsername(string $username): ?User{
        $resultObj = null;
        //get all data
        $objList = $this->selectAll();
        //enter for for iterate users
        for ($i=0; $i<count($objList); $i++) {
            //If user equals add user to resultObj
            if ($objList[$i]->getUsername() === $username) {
                $resultObj = $objList[$i];
            }
        }
        return $resultObj;
     ...
     * @param $usurname username of user
     * @param $password password of user
     * @return $resultObj user returned with these username
     */
    public function searchUsersbyUsernameAndPassword(string $username, string $password): ?User{
        $resultObj = null;
        //get all data.
        $objList = $this->selectAll();
        //enter for for iterate users
        for ($i=0; $i<count($objList); $i++) {
            //if user and password equals add user to resultObj
            if($objList[$i]->getUsername() === $username && $objList[$i]->getPassword() === $password){
                $resultObj = $objList[$i];
            }
        }
        return $resultObj;
