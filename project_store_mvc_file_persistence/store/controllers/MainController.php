                        ...
                        if($role == "admin" || $role == "staff" || $role == "user"){
                            ...
                            if(filter_var($name,FILTER_VALIDATE_REGEXP,array("options" => array("regexp"=>"/^[A-Za-z 0-9]+$/")))){
                                ...
                                if(filter_var($surname,FILTER_VALIDATE_REGEXP,array("options" => array("regexp"=>"/^[A-Za-z 0-9]+$/")))){
                                    //Search if exist user with same id
                                    $userFoundId = $this->model->searchUserById($id);
                                    //If user with same id not found enter if
                                    if(is_null($userFoundId)){
                                        ...
                                        if(is_null($userFoundUsername)){
                                            ...
                                            $user = new User($id,$username,$password,$role,$name,$surname);
                                        ...
                                        //If user found with same id send to user-form page with error message
                                        $data['message'] = "This id is in other user, please chose other id";
                                        $userForm = new User($id, $username, $password, $role, $name, $surname);
                                        $data['user'] = $userForm;
                                        $this->view->show("user/user-form.php", $data);
                                    }
                            ...
                            $data['message'] = "Role only can be admin/staff/user";
                            $userForm = new User($id, $username, $password, $role, $name, $surname);
                            $data['user'] = $userForm;
                            $this->view->show("user/user-form.php", $data);
                        }
                        ...
                        if($role == "admin" || $role == "staff" || $role == "user"){
                            ...
                            if(filter_var($name,FILTER_VALIDATE_REGEXP,array("options" => array("regexp"=>"/^[A-Za-z 0-9]+$/")))){
                                ...
                                if(filter_var($surname,FILTER_VALIDATE_REGEXP,array("options" => array("regexp"=>"/^[A-Za-z 0-9]+$/")))){
                                    ...
                                    if(!is_null($userID)){
                                        //Delete user with this id for search if other user have same username
                                        $usersDeleted = $this->model->deleteUser($id);
                                        //Search if user with same username exist
                                        $userFoundusername = $this->model->searchUsersByUsername($username);
                                        //Add user deleted
                                        $userAdded = $this->model->addUser($userID);
                ...
                if($price != false && $price > 0){
                    ...
                    if($stock != false && $stock > 0){
                         ...
                         if(is_null($ProductFoundId)){
                            ...
                            //If product not added correctly send to product-form page with error message
                            if($ProductAdded  == 0){
                                $data['message'] = "Product not added, check it please";
                                $ProductForm = new Product($id, $description, $price, $stock);
                                $data['product'] = $ProductForm;
                                $this->view->show("product/product-form.php", $data);
                            } else{
                                //If product added correctly send to product-form page with info message
                                ...
                                //If product found with same id send to product-form page with error message
                                $data['message'] = "This id is in other product, please chose other id";
                                $ProductForm = new Product($id, $description, $price, $stock);
                                $data['product'] = $ProductForm;
                                $this->view->show("product/product-form.php", $data);
                         }
